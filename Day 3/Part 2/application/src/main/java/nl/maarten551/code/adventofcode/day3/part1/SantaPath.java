...
public class SantaPath {
    ...
    public HashMap<String, Integer> calculateTakenRoute(String pathTaken) {
        ...
        Dimension currentSantaPosition = new Dimension(0, 0);
        Dimension currentSantaRobotPosition = new Dimension(0, 0);

        this.addOrUpdateVisitedHouses(visitedHouses, currentSantaPosition); //Present is given to first house
        this.addOrUpdateVisitedHouses(visitedHouses, currentSantaRobotPosition);

        for(Character directionChar : pathTaken.toCharArray()) {
            Dimension currentSantaEntityDimension = (isSantaTurn) ? currentSantaPosition : currentSantaRobotPosition;
            ...
            if (takenDirection != null) {
                this.updateDimensionByTakenDirection(currentSantaEntityDimension, takenDirection);
                this.addOrUpdateVisitedHouses(visitedHouses, currentSantaEntityDimension);
            }
        }
