...
public class LeftShiftOperator extends Operator {
    private static final Pattern commandDissectRegex = Pattern.compile("^([a-z0-9]{1,5}) LSHIFT ([a-z0-9]{1,5}) -> ([a-z]{1,2})$");
    ...
    public boolean compileCommand(String command) {
        Matcher matcher = LeftShiftOperator.commandDissectRegex.matcher(command);
        if(matcher.find()) {
    ...
    protected Pattern getCommandDissectRegex() {
        return LeftShiftOperator.commandDissectRegex;
    }
}
